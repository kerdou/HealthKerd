...
/** Construction puis affichage de la page contenant tous les events (passés et futurs) d'un docteur
 */
class EventsWithOneDocPageBuilder extends \HealthKerd\View\common\ViewInChief
{
    ...
    private function pageElementsSettingsList(): void
    {
        $this->pageSettingsList = array(
            ...
            "pageTitle" => '&Eacute;vénements médicaux liés à un professionnel de santé',
    ...
    private function allEventsBuilder(array $eventsData): void
    {
        if ((sizeof($eventsData['futureEvents']) == 0) && (sizeof($eventsData['pastEvents']) == 0)) {
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/doc/eventsWithOneDoc/noEventsContent.html');
        } else {
            ...
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/doc/eventsWithOneDoc/eventsContent.html');
    ...
    private function contentElementsSettingsList(): void
    {
        $this->contentSettingsList = array(
            'mainContent' => file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/doc/eventsWithOneDoc/eventsWithOneDoc.html'),
