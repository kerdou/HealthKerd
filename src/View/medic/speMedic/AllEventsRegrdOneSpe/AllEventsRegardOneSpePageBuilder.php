...
namespace HealthKerd\View\medic\speMedic\AllEventsRegrdOneSpe;

/** Construction puis affichage de la page contenant tous les events (passés et futurs) d'une spécialité médicale
 */
class AllEventsRegardOneSpePageBuilder extends \HealthKerd\View\common\ViewInChief
{
    ...
    private function allEventsBuilder(array $eventsData): void
    {
        if ((sizeof($eventsData['futureEvents']) == 0) && (sizeof($eventsData['pastEvents']) == 0)) {
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/speMedic/allEventsRegardingOneSpe/noEventsContent.html');
        } else {
            ...
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/speMedic/allEventsRegardingOneSpe/eventsContent.html');
