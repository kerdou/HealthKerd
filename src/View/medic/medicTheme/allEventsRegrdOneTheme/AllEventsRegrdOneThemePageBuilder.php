...
class AllEventsRegrdOneThemePageBuilder extends \HealthKerd\View\common\ViewInChief
{
    ...
    private function allEventsBuilder(array $eventsData): void
    {
        if ((sizeof($eventsData['futureEvents']) == 0) && (sizeof($eventsData['pastEvents']) == 0)) {
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/medicTheme/allEventsRegardingOneTheme/noEventsContent.html');
        } else {
            ...
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/medicTheme/allEventsRegardingOneTheme/eventsContent.html');
    ...
    private function contentElementsSettingsList(): void
    {
        $this->contentSettingsList = array(
            'mainContent' => file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/medicTheme/allEventsRegardingOneTheme/allEventsRegardingOneTheme.html'),
