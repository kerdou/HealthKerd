...
class AllEventsRegrdOneDocOfficePageBuilder extends \HealthKerd\View\common\ViewInChief
{
    ...
    private function allEventsBuilder(array $eventsData): void
    {
        if ((sizeof($eventsData['futureEvents']) == 0) && (sizeof($eventsData['pastEvents']) == 0)) {
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/docOffice/allEventsRegardingOneDocOffice/noEventsContent.html');
        } else {
            ...
            $this->eventsContent = file_get_contents($_ENV['APPROOTPATH'] . 'templates/loggedIn/medic/docOffice/allEventsRegardingOneDocOffice/eventsContent.html');
